...
**With this fork the polyfill will also be applied if there is native `Promise` support but `Promise.prototype.finally` is missing. ASAP is also updated to only use `setTimeout`, because some of the other options crashed old devices. `Promise.all` and `Promise.race` will also be polyfilled if missing. The readme has not been updated on this fork other than adding this message.**
